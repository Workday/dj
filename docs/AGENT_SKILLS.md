...
This page explains what the skills are, how to turn them on, and catalogs the 12 skills DJ provides today.
...
- **Just ask.** Describe your task in natural language ("create a mart for daily orders", "why is this query slow?") and the assistant matches it to a skill via that skill's *Use when…* description. You can also name a skill directly.
...
- **Some skills are read-only.** `dj-review-python-model` and `dj-trino-analyzer` produce reports and change nothing.

## The skills

DJ provides 12 skills, grouped below by what they help you do.
...
- **Example prompt:** *"Set up the DJ framework in this dbt project."*
...
- **Example prompt:** *"Create a mart that summarizes daily order totals per customer."*
...
- **Example prompt:** *"Convert this draft.sql into a DJ model."*
...
- **Example prompt:** *"Create a Python model that fetches the Backstage API into an Iceberg table."*
...
- **Example prompt:** *"Review this Python model for production readiness."*
...
- **Example prompt:** *"Build a Lightdash dashboard for the customer_orders explore."*
...
- **Example prompt:** *"Change this chart's date filter to the last 30 days."*
...
- **Example prompt:** *"Update the AI hints for this model's dependency tree from ai_hints.xlsx."*
...
- **Example prompt:** *"Review and modernize the models in this folder."*
...
- **Example prompt:** *"Inline the ephemeral models under intermediate/ as CTEs."*
...
- **Example prompt:** *"Resolve the merge conflicts in these .model.json files."*
...
- **Example prompt:** *"Explain why Trino query 20260101_120000_00001_abcde is slow."*

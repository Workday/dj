...
- **`group`** must be a **registered** group — read `models/groups.yml` (or the `dbt_project.yml` models config) for the valid set. If the requested group isn't registered, ask the user to pick a registered one or register a new one (via `dj-initialize`); never invent a group or create an unregistered folder.

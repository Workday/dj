...
| Invoke DJ operations via the CLI bridge (`.dj/bin/dj`) — discovery, authoring, compile, query                                             | `.agents/skills/dj-cli/SKILL.md` (skill/CLI routing: `dj-cli-registry`)    |
...
- **Three-tier discovery and execution.** (1) **DJ CLI bridge** — when `.dj/bin/dj system.ping` succeeds, prefer `.dj/bin/dj <op>` (e.g. `trino.columns`, `model.preview`, `dbt.compile`) over hand-authoring JSON, loading `.dj/schemas/`, grepping for columns, or raw `trino-cli`. Load `dj-cli` for invocation patterns and `dj-cli-registry` to route tasks. (2) **Read project files** — `.source.json` / `.model.json` / `target/manifest.json` when the bridge is unavailable or has no op for the task. (3) **Raw `trino-cli` / `dbt` CLI** — last resort; still subject to read-only / confirmation rules below.

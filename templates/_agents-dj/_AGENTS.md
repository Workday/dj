...
This file holds the **always-relevant** rules: project structure, naming, structural governance, project/environment resolution, command safety, and the core conventions. Read it first.

The deep, topic-specific material lives in **`reference/` files that you load on demand** — only when the task calls for it. Pick the file that matches what you're doing:
...
You can also read existing `.model.json` and `.source.json` files in the `models/` directory for real examples from this project.

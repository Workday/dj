...
- **Rollup on select/join**: Optional **`rollup`** on **`from.model`** for **`int_select_model`** and **`int_join_models`** only (not marts). Keeps a normal **`select`** / join; coarser **`interval`** triggers **re-aggregation** of declarative **`agg`/`aggs`**. **`model.from.rollup.schema.json`**. For **`group_by` / `agg` / `expr`** rules see `.agents/dj/AGENTS.md` **Important Conventions** (#9–#10).
